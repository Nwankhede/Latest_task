...
object data_etl {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val salesDF = readData(spark, "src/main/resources/sales_data.csv", salesSchema).get //.get fetches the data into
      logger.debug("Sales data read successfully")
      val productDF = readData(spark, "src/main/resources/product_data.csv", productSchema).get
  ...
  def readData(spark: SparkSession, filePath: String, schema: StructType): Option[DataFrame] = {
    val filepathdir = new File(filePath)
    if (filepathdir.exists()){
      ...
      Some(df) // options data frame has been created
